...
export async function POST(req: NextRequest) {
  ...
  // fetch existing env vars once so we can upsert
  const existingRes = await fetch(
    `https://api.vercel.com/v9/projects/${projectId}/env?limit=100`,
    { headers: { Authorization: `Bearer ${vercelToken}` } }
  );
  const existingData = existingRes.ok ? await existingRes.json().catch(() => ({})) : {};
  const existingEnvs: Array<{ id: string; key: string; target: string[] }> = existingData?.envs ?? [];

  // push each var to Vercel API — create or update if already exists
  for (const [key, value] of Object.entries(envVars)) {
    ...
    if (!res.ok) {
      ...
      return NextResponse.json({ error: message || `failed to set ${key} (${res.status})` }, { status: 500 });
    }
  }
