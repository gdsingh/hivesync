...
export function Dashboard({
  ...
  const [syncCountAudit, setSyncCountAudit] = useState<SyncCountAudit | null>(
    isPreview
      ? {
          firstYear: FIRST_FOURSQUARE_YEAR,
          lastYear: currentYear,
          checkedYears: {},
          totalCheckins: previewFoursquareTotal ?? 3241,
          totalSynced,
          foundInCheckedYears: totalSynced,
          unaccountedCheckins: 14,
          completed: true,
          checkedAt: new Date("2026-05-13T00:00:00Z"),
        }
      : null
  );
  const [googlePlacesStatus, setGooglePlacesStatus] = useState<GooglePlacesStatus | null>(
    isPreview
      ? {
          hasKey: true,
          dailyLimit: 25,
          monthlyLimit: 500,
          backfillRunLimit: 250,
  ...
  useEffect(() => {
    ...
    if (isPreview) {
      const sampleFoursquare = yr <= 2011 ? 160 + (yr - 2009) * 90 : yr < 2020 ? 220 : 120;
      const sampleSynced = Math.max(0, sampleFoursquare - (yr <= 2011 ? 12 : 0));
      setYearCounts((prev) => ({ ...prev, [fromYear]: { foursquare: sampleFoursquare, synced: sampleSynced } }));
      setYearCountLoading(false);
      return;
    }
  ...
  useEffect(() => {
    ...
    if (isPreview) {
      setRangeCount({ foursquare: rangeTo ? 17 : 3, synced: rangeTo ? 15 : 2 });
  ...
  async function handleQuickSync() {
    ...
    setQuickError(null);
    if (isPreview) {
      setQuickResult({ synced: Math.min(Number(quickCount) || 0, 3), skipped: Math.max(0, (Number(quickCount) || 0) - 3) });
  ...
  async function handleQuickDelete() {
    ...
    setQuickResult(null);
    if (isPreview) {
      setQuickDeleteResult({ deleted: Number(quickDeleteCount) || 0, errors: 0 });
  ...
  async function handleDeleteYear() {
    ...
    if (isPreview) {
      setDeleteYearResult({ deleted: yearCounts[deleteYearValue]?.synced ?? 0, errors: 0 });
  ...
  async function saveGooglePlacesLimits() {
    ...
    if (isPreview) {
      setGooglePlacesStatus((prev) => prev ? {
        ...prev,
        dailyLimit: parseInt(googlePlacesLimitDraft.dailyLimit, 10) || prev.dailyLimit,
        monthlyLimit: parseInt(googlePlacesLimitDraft.monthlyLimit, 10) || prev.monthlyLimit,
        backfillRunLimit: parseInt(googlePlacesLimitDraft.backfillRunLimit, 10) || prev.backfillRunLimit,
  ...
  async function runGooglePlacesPreflight(afterTimestamp: number, beforeTimestamp: number): Promise<GooglePlacesPreflight | null> {
    ...
    if (isPreview) {
      const estimate = 12;
      const limit = getManualGooglePlacesLimit();
      const preflight = {
        estimatedGoogleCalls: estimate,
        uniqueUncachedVenues: estimate,
        dailyRemaining: 25,
        monthlyRemaining: 500,
        backfillRemaining: limit,
        fallbackCount: Math.max(0, estimate - limit),
        confirmationRequired: false,
      };
      setGooglePlacesPreflight(preflight);
  ...
  async function startRangeSync(googlePlacesApproved: boolean, fromDate: Date, toDate?: Date) {
    ...
    if (isPreview) {
      const total = rangeCount?.foursquare ?? 3;
      setRangeSyncState({ type: "done", synced: Math.min(total, 2), skipped: Math.max(0, total - 2), errors: 0 });
      return;
    }
  ...
  async function handleRangeDelete() {
    ...
    if (isPreview) {
      setRangeDeleteResult({ deleted: rangeCount?.synced ?? 0, errors: 0 });
  ...
  async function startFullSync(googlePlacesApproved: boolean, yearValue: string) {
    ...
    setDeleteYearResult(null);
    if (isPreview) {
      const total = yearCounts[yearValue]?.foursquare ?? 24;
      setSyncState({ type: "done", synced: Math.min(total, 8), skipped: Math.max(0, total - 8), errors: 0 });
      return;
    }
    ...
    <div className="max-w-2xl mx-auto px-4 py-12 space-y-8">
                      ...
                      {/* foursquare */}
                        ...
                        <div className="shrink-0">
                          {foursquareConnected && !foursquareDisconnected ? (
                            ...
                            <Button size="sm" asChild><a href={sitePath("/api/auth/foursquare/connect")}>connect</a></Button>
                          )}
                        </div>
                      </div>
      ...
      {!foursquareConnected && !isPreview && (
        <Alert className="flex items-center justify-between gap-4 border-amber-500/50 bg-amber-50 dark:bg-amber-950/20 text-amber-900 dark:text-amber-200">
          ...
          <Button size="sm" asChild className="shrink-0 bg-[#1C2729] hover:bg-[#1C2729]/90 text-white border-0"><a href={sitePath("/api/auth/foursquare/connect")}>Get Started</a></Button>
        </Alert>
      )}
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          ...
          <span className="text-muted-foreground/30">·</span>
          <form action={sitePath("/api/auth/logout")} method="POST" className="flex">
