...
export function AppHeader({ lastSyncedAt, rightExtra }: Props) {
  ...
  const prefix = isPreview ? "/preview" : isDemo ? "/demo" : "";
  ...
  const badge = relative ? (
    <a href={sitePath(`${prefix}/history`)} className="group relative inline-flex items-center px-1.5 py-0.5 rounded-full bg-muted text-muted-foreground text-[10px] font-bold tracking-tight hover:bg-muted/70 transition-colors">
  ...
  return (
    <div className="flex flex-col gap-3 pb-4 sm:flex-row sm:items-center sm:justify-between">
      <a href={sitePath(`${prefix}/home`)} className="inline-flex items-center gap-2 hover:opacity-70 transition-opacity">
