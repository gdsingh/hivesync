...
import { LuCrown, LuChevronDown, LuX, LuCalendarDays, LuRefreshCw, LuTrash2, LuCalendarRange, LuCalendar, LuGithub, LuPlug, LuList } from "react-icons/lu";
...
export function Dashboard({
  ...
  async function handleQuickSync() {
    if (isPreview) {
      ...
      setQuickResult({ synced: Number(quickCount) || 5, skipped: 0 });
      return;
    }
  ...
  async function handleQuickDelete() {
    if (isPreview) {
      ...
      setQuickDeleteResult({ deleted: Number(quickDeleteCount) || 5, errors: 0 });
      return;
    }
    setQuickDeleteLoading(true);
    setQuickDeleteResult(null);
  ...
  async function handleDeleteYear() {
    if (isPreview) {
      ...
      setDeleteYearResult({ deleted: 47, errors: 0 });
      return;
    }
    setDeleteYearLoading(true);
  ...
  async function handleRangeSync() {
    ...
    if (isPreview) {
      ...
      setRangeSyncState({ type: "done", synced: 12, skipped: 3, errors: 0 });
      return;
    }
  ...
  async function handleRangeDelete() {
    ...
    if (isPreview) {
      ...
      setRangeDeleteResult({ deleted: 12, errors: 0 });
      return;
    }
    setRangeDeleteLoading(true);
    setRangeDeleteResult(null);
  ...
  async function handleFullSync() {
    if (isPreview) {
      ...
      await new Promise((r) => setTimeout(r, 1000));
      setSyncState({ type: "done", synced: 38, skipped: 5, errors: 0 });
      return;
    }
    stopRangeSyncRef.current = true;
    setSyncState({ type: "loading" });
    ...
    <div className="max-w-2xl mx-auto px-4 py-12 space-y-8">
                        ...
                        <div className="shrink-0">
                          {foursquareConnected ? (
                            pendingDisconnect ? (
                              ...
                              <button onClick={() => setPendingDisconnect(true)} disabled={isPreview} className="inline-flex items-center px-2 py-0.5 rounded-full text-xs bg-red-100 text-red-700 hover:bg-red-200 dark:bg-red-900/30 dark:text-red-400 dark:hover:bg-red-900/50 transition-colors disabled:opacity-40 disabled:cursor-default">disconnect</button>
                            )
                          ) : (
      ...
      <div className="space-y-4">
        <p className="text-xs font-medium text-muted-foreground uppercase tracking-wider flex items-center gap-1.5">
          <span className="group relative w-[13px] h-[13px] flex items-center justify-center">
        ...
        <div className="flex items-center">
          <div>
            ...
            <p className="text-xs text-muted-foreground flex items-center gap-1">
              ...
              {calendarStatus === "needs_rename" && `click to rename calendar to ${ogMode ? "Foursquare" : "Swarm"}`}
              {calendarStatus === "missing" && <><span className="text-amber-600 dark:text-amber-500">⚠️ google calendar not found</span> —{" "}<button onClick={handleCalendarSetup} disabled={calendarSetupLoading} className="inline-flex items-center px-1.5 py-0.5 rounded-full bg-foreground text-background hover:opacity-80 transition-opacity disabled:opacity-50">{calendarSetupLoading ? "creating…" : "create it to enable sync"}</button></>}
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          ...
          <form action="/api/auth/logout" method="POST" className="flex">
            <Button type="submit" variant="outline" size="sm" className="h-7 text-xs px-3" disabled={isPreview}>sign out</Button>
          </form>
        </div>
      </div>
